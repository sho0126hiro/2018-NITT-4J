...
static VP kernel_tsk4_stk[((256) + sizeof(VP) - 1) / sizeof(VP)];

/* task control block for rom area */
const T_KERNEL_TCB_ROM kernel_tcb_rom[4] =
	{
		...
		{(ATR)(TA_HLNG), (VP_INT)(1), (FP)(taskC), (PRI)(3), (SIZE)(256), (VP)kernel_tsk4_stk},
	};

/* task control block for ram area */
T_KERNEL_TCB_RAM kernel_tcb_ram[4];
...
static VP kernel_mpf1_heap[1][(((256) + sizeof(VP) - 1) / sizeof(VP))];

/* fixed size memory-pool control block for rom area */
const T_KERNEL_MPFCB_ROM kernel_mpfcb_rom[1] =
	{
		{(ATR)(TA_TPRI), (UINT)(1), (SIZE)sizeof(kernel_mpf1_heap[0]), (VP)kernel_mpf1_heap},
	};
