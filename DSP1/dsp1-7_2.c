...
//FFT or IFFT
void FastFourieTransform(complex Y[DATASIZE],complex X[DATASIZE],int mode){
...
int main(){
    double input[DATASIZE] = {1,-1*sqrt(2),-3,-1*sqrt(2),1,sqrt(2),1,sqrt(2)}; // 入力データ
    complex c_input[DATASIZE];    // 入力データ >> 複素数
    complex c_output[DATASIZE];   // 出力データ >> 複素数
    complex c_output2[DATASIZE];   // ifft result
    ...
    // fft
    printf("============= fft =================\n");
    FastFourieTransform(c_output,c_input,0);
    // ifft
    printf("============= ifft =================\n");
    FastFourieTransform(c_output2,c_output,1);
    printf("============= ifft-Display =================\n");
    DisplayComplex(c_output2);
    // amplitude spectrum
