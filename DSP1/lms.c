...
#include <time.h>
#define ALPHA 0.95
int _wn_size=50; //wnのファイルサイズ (=xn fileSize)
...
void lsm(double *xn,double *hn,double *wn){
    ...
    // fileWrite
    FileWrite("error.txt",error,_sample);
    FileWrite("ara11_e.txt",_e,_sample);
    FileWrite("x.txt",_x0,_sample);
    FileWrite("d.txt",_y,_sample);
    FileWrite("y.txt",_d,_sample);
}
