...
//相関係数を求める
//注意：配列をIMAXで初期化するため，読み込みファイル内にIMAXが含まれている場合
//      使用できない．
//      atoi関数を使用しているため、読み込みファイル内の値はオーバーフローする値を取ってはならない。
//      -2147483649~2147483648

#define DATASIZE 256 //data size
...
void init_data(int data[],int length){
	for(int i=0;i<length;i++)data[i]=IMAX;
}
...
//相関係数の算出
void Calculation(double data1[],double data2[]){
	/*算出手順
...
void misCalculation(double data1[],double data2[]){
	int i=0;
	double d1_a_sub[DATASIZE];//平均値を引いたデータ
	double d2_a_sub[DATASIZE];
	init_double_data(d1_a_sub,DATASIZE);
...
int main(){
	...
	//Signature();
	//人口データ格納配列
	double data1[DATASIZE]={0};//H16男性人口
	double data2[DATASIZE]={0};//H16女性人口
	double data3[DATASIZE]={0};//H16総人口
	double data4[DATASIZE]={0};//S23総人口
	/*
	//各配列の要素数を取得
	...
	*/
	//読み込みファイル名
	char fname1[]="降水量.txt";
	char fname2[]="最高気温.txt";
	char fname3[]="最低気温.txt";
	char fname4[]="平均気温.txt";
	
	//dataを全て初期化（いるかまだ分からない）
	init_double_data(data1,DATASIZE);
	init_double_data(data2,DATASIZE);
	init_double_data(data3,DATASIZE);	
	init_double_data(data4,DATASIZE);
	...
	printf(" %s と %s の ",fname2,fname3);
	Calculation(data2,data3);
	printf(" %s と %s の ",fname2,fname4);
	Calculation(data2,data4);
	printf(" %s と %s の ",fname3,fname4);
	Calculation(data3,data4);
	return 0;	
}
