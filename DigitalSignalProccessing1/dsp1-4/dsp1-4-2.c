...
#define DATASIZE 4001 //相互相関係数　data size
#define DATASIZE_2 4001 //自己相関係数　data size
...
int main(){
	...
	FileRead("Kumamoto.txt",data1);
	FileRead("Yatsushiro.txt",data2);
	//格納終了
	double Rxy[DATASIZE-1]={0};//相互相関係数格納用配列
	CrossCorrelation(data1,data2,Rxy);
	FileWrite("Rxy-2.txt",Rxy,DATASIZE);
	*/
	double data3[DATASIZE_2]={0};//c.txt
	FileRead("Kumamoto.txt",data3);
