...
public class NonlinearEquation{
    ...
    public static final double ALPHA3  = 2.95; //  f3(x)に含まれるalpha (kadai11)
    public static final double POSITIVE_MAX = 4.0;
    public static final double NEGATIVE_MAX = 0.0;
    public static final double DELTA_X = 0.0001; // 微分を差分で求める式の中のΔxの値
    ...
    public double function3(double x){
        return Math.exp(x)-ALPHA3*x;
    }
    /**
     *  kadai11で与えられた関数(微分：手計算)
     */
    public double function3_diff(double x){
        return Math.exp(x)-ALPHA3;
    }
    /**
     *  kadai11 前進差分の式を用いて微分の値を求める
     */
    public double function3_diff2(double x){
        return (function3(x+DELTA_X)-function3(x))/DELTA_X;
    }
    ...
    /**
     * Newton法によって非線形方程式を解く
     */
    public void _solveNLEByNewton(){
        double value;      // x_{n} に対応（初回のvalue_min = x_0とする）
        double nextValue;
        value = 1.19;
        while(true){
            nextValue = value - function3(value)/function3_diff2(value); 
            // 終了判定
            if(Math.abs(nextValue - value) < EPSILON || this.iteration_ >= MAXIMUM_IT){
                this.answer_ = nextValue;
                break;
            }
            System.out.println("xNext = "+nextValue + ", f(xNext)="+function2(nextValue));
            value = nextValue;
            this.iteration_ += 1;
        }
    }
